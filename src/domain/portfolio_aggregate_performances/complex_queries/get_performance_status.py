...
def get_performance_status(portfolio_aggregate_id, status_date=None):
    cte_dates = (
        select(
            PortfolioAggregatePerformance.portfolio_aggregate_id,
            PortfolioAggregatePerformance.date,
            (
                func.lag(PortfolioAggregatePerformance.date).over(
                    order_by=PortfolioAggregatePerformance.date.asc()
                )
            ).label("previous_date"),
            func.max(PortfolioAggregatePerformance.date).over().label("max_date"),
        ).where(
            PortfolioAggregatePerformance.portfolio_aggregate_id
    ...
    curr_perf = aliased(PortfolioAggregatePerformance, name="curr_perf")
    prev_perf = aliased(PortfolioAggregatePerformance, name="prev_perf")
    ...
    query = (
        ...
        .join_from(
            ...
            curr_perf,
            and_(
                curr_perf.portfolio_aggregate_id
                == cte_status_dates.c.portfolio_aggregate_id,
        ...
        .join(
            prev_perf,
            and_(
                prev_perf.portfolio_aggregate_id
                == cte_status_dates.c.portfolio_aggregate_id,
