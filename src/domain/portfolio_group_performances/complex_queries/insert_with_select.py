...
def insert_with_select(user_id: int):
    cte_1 = (
        select(
            ...
            func.sum(PortfolioAssetPerformance.market_value).label("market_value"),
            func.sum(PortfolioAssetPerformance.market_value_adj).label(
                "market_value_adj"
            ),
            func.sum(PortfolioAssetPerformance.delta_quantity_value_adj).label(
                "delta_quantity_value_adj"
            ),
            func.sum(PortfolioAssetPerformance.invested_amount).label(
                "invested_amount"
            ),
            func.sum(PortfolioAssetPerformance.invested_amount_total).label(
                "invested_amount_total"
            ),
            func.sum(PortfolioAssetPerformance.asset_disposal_income).label(
                "asset_disposal_income"
            ),
            func.sum(PortfolioAssetPerformance.asset_disposal_income_total).label(
                "asset_disposal_income_total"
            ),
            func.sum(PortfolioAssetPerformance.asset_holding_income).label(
                "asset_holding_income"
            ),
            func.sum(PortfolioAssetPerformance.asset_holding_income_total).label(
                "asset_holding_income_total"
            ),
            func.sum(PortfolioAssetPerformance.investment_income).label(
                "investment_income"
            ),
            func.sum(PortfolioAssetPerformance.investment_income_total).label(
                "investment_income_total"
            ),
            func.sum(PortfolioAssetPerformance.profit).label("profit"),
            func.sum(PortfolioAssetPerformance.profit_total).label("profit_total"),
        )
        .join_from(
            PortfolioAssetPerformance,
            Portfolio,
        ...
        .union_all(
            select(
                ...
                PortfolioGroupPerformance.market_value_adj,
                PortfolioGroupPerformance.delta_quantity_value_adj,
                PortfolioGroupPerformance.invested_amount,
                PortfolioGroupPerformance.invested_amount_total,
                PortfolioGroupPerformance.asset_disposal_income,
                PortfolioGroupPerformance.asset_disposal_income_total,
                PortfolioGroupPerformance.asset_holding_income,
                PortfolioGroupPerformance.asset_holding_income_total,
                PortfolioGroupPerformance.investment_income,
                PortfolioGroupPerformance.investment_income_total,
                PortfolioGroupPerformance.profit,
                PortfolioGroupPerformance.profit_total,
            )
            .join_from(
                PortfolioGroupPerformance,
                Portfolio,
    ...
    cte_2 = select(
        ...
        .over(
            partition_by=[cte_1.c.portfolio_id, cte_1.c.portfolio_group_id],
        ...
        (
            func.exp(
                func.sum(func.ln(1 + cte_2.c.hpr)).over(
                    partition_by=[cte_2.c.portfolio_id, cte_2.c.portfolio_group_id],
    ...
    cte_4 = select(
        ...
        (
            func.max(cte_3.c.hpr_cumulative).over(
                partition_by=[cte_3.c.portfolio_id, cte_3.c.portfolio_group_id],
    ...
    query = select(
        cte_4.c.portfolio_id,
        cte_4.c.portfolio_group_id,
        cte_4.c.date,
    ...
    return insert(PortfolioGroupPerformance).from_select(
        [
            "portfolio_id",
            "portfolio_group_id",
            "date",
