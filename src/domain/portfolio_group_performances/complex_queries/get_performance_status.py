...
def get_performance_status(portfolio_group_id, status_date=None):
    cte_dates = (
        select(
            PortfolioGroupPerformance.portfolio_group_id,
            PortfolioGroupPerformance.date,
            (
                func.lag(PortfolioGroupPerformance.date).over(
                    order_by=PortfolioGroupPerformance.date.asc()
                )
            ).label("previous_date"),
            func.max(PortfolioGroupPerformance.date).over().label("max_date"),
        ).where(PortfolioGroupPerformance.portfolio_group_id == portfolio_group_id)
    ).cte("cte_dates")
    ...
    curr_perf = aliased(PortfolioGroupPerformance, name="curr_perf")
    prev_perf = aliased(PortfolioGroupPerformance, name="prev_perf")
    ...
    query = (
        ...
        .join_from(
            ...
            curr_perf,
            and_(
                curr_perf.portfolio_group_id == cte_status_dates.c.portfolio_group_id,
        ...
        .join(
            prev_perf,
            and_(
                prev_perf.portfolio_group_id == cte_status_dates.c.portfolio_group_id,
