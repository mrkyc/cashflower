...
*   **Dividend Income:** Income received from dividends.
*   **Interest Income:** Income received from interest.
*   **Investment Income:** The total income generated from investments, including asset disposals, dividends, and interest.
...
*   **transaction_type:** Type of transaction. Supported types are: "buy", "sell", "deposit", "withdrawal", "dividend", "interest", "fee".
    For these transaction types, and only for these, the corresponding fields will be considered. The system automatically handles the sign of the values (positive or negative) based on the transaction type; you can provide either absolute values or values with signs in your file. The interpretation of quantity, transaction value, fee, and tax depends on the transaction type:
    *   **quantity:**
        *   `buy`: The quantity will be treated as positive.
        *   `sell`: The quantity will be treated as negative.
    *   **transaction_value:**
        *   `deposit`, `sell`, `dividend`, `interest`: The value will be treated as positive.
        *   `withdrawal`, `buy`: The value will be treated as negative.
    *   **fee_amount:**
        *   `deposit`, `withdrawal`, `sell`, `dividend`, `interest`, `buy`, `fee`: The amount will be treated as negative.
    *   **tax_amount:**
        *   `sell`, `dividend`, `interest`: The amount will be treated as negative.
...
*   **Show total values (including fees paid):** Option to enable/disable the display of total values (including fees paid).
